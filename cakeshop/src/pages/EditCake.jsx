...
import { cakeValidationSchema } from "../models/cakeValidationSchema";
...
import { updateCake } from "../services/apiCake";
...
const EditCake = () => {
  ...
  const { cakes, setCakes, categories } = useContext(CakeContext);
  const { id } = useParams();
  ...
  const category = categories.find(
    (category) => category.id === cake.categoryId,
  );
  ...
  const initialCakeValues = {
    ...
    UserId: cake.userId,
    CakeOptions: cake.cakeOptions,
    CakeImages: cake.cakeImages.map((img) => ({
      id: uuid(),
      src: img,
      file: null,
    })),
    ExistingCakeImages: cake.cakeImages,
    IsPromoted: cake.isPromoted,
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    values.ExistingCakeImages.forEach((item) =>
      formData.append("ExistingCakeImages", item),
    );

    const result = await updateCake(id, formData, jwt);

    if (result.success) {
      console.log("result", result.data);
      // Replace the updated cake in the cakes array
      setCakes((prev) =>
        prev.map((cake) => (cake.id === id ? result.data : cake)),
      );
      navigate("/manage-cake");
    } else {
      console.error("Error updating cake");
    }
                ...
                >
                  {categories.map((category) => (
                    <option
                      key={category.id}
                      value={category.id}
                      selected={category.id === values.CategoryId}
                    >
